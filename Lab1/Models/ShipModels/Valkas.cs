...
public class Valkas : IShip
{
    private const int VaklasMultiplier = 2;
    private readonly IHullStrength _hullStrength;

    public Valkas(bool havePhotonDeflector)
    {
        Deflector = havePhotonDeflector ? new PhotonDeflectorDecorator(new DeflectorType1()) : new DeflectorType1();
        JumperEngine = new JumperEngineGammaClass();
        ImpulseEngine = new ImpulseEngineEClass(VaklasMultiplier);
        _hullStrength = new HullStrengthClass2(VaklasMultiplier);
