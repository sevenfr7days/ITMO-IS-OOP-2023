...
public class NormalSpace : ISpaceEnvironment
{
    ...
    private IReadOnlyCollection<INormalSpaceObstacle> _obstaclesCollection;

    public NormalSpace(LightHours length, params INormalSpaceObstacle[] obstacles)
    {
        _length = length;
        ...
        params IFuel[] fuels)
    {
        ...
        if (pathResult is EnvironmentFlightResult.Success success)
        {
            ...
            foreach (INormalSpaceObstacle obstacle in _obstaclesCollection)
            {
